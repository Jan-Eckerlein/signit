...
use App\Mail\DocumentCompletedMagicLinkMailable;
...
class SendDocumentCompletedNotificationListener
{
    /**
        ...
        foreach ($event->document->documentSigners as $documentSigner) {
            ...
            if ($documentSigner->user->isAnonymous()) {
                ...
                Mail::to($documentSigner->user->email)
                    ->queue(new DocumentCompletedMagicLinkMailable($event->document, $documentSigner->user, $token));
            } else {
                // For regular users, send standard email notification

                Mail::to($documentSigner->user->email)
                    ->queue(new DocumentCompletedMailable($event->document, $documentSigner->user));
            }
        }
    }
}
